...
@Component({
  ...
  template: `
      ...
      <form #f="ngForm">
        ...
        <div class="form-group">
          ...
          <select
            ...
            [ngModel]="displayTz"
            (ngModelChange)="changeDisplayTz($event)"
          >
    ...
    <br />
    <div>
      Tenant/Device time zone: <strong>{{ selectedTz }}</strong>
    </div>
    <div>
      User's time zone: <strong>{{ userTz }}</strong>
    </div>
    <br />
    <div>UTC Date: {{ date }}</div>
    <br />
    <div>
      Date displayed in: <strong>{{ displayTz }}</strong>
    </div>
    <div>
      Unformated: <strong>{{ a }}</strong>
    </div>
    <div>
      Formated: <strong>{{ aFormat }}</strong>
    </div>
    <div>
      Date formated: <strong>{{ aDateFormat }}</strong>
    </div>
    <div>
      Time formated: <strong>{{ aTimeFormat }}</strong>
    </div>
    <div>
      Special format: <strong>{{ aSpecialFormat }}</strong>
    </div>
    <br />
    <div>
...
export class AppComponent {
  ...
  private num = 1623724199000;

  private locales = ['ar', 'en', 'en-gb', 'fr', 'es', 'de', 'hi'];
  ...
  private updateTime(timeZone: string) {
    ...
    this.tenantDateTimeFormatted = this.timeZoneService.utcToTenantString(
      this.date,
      'LLL'
    );
  }
